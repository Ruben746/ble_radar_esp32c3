...
Source : firmware transmis par l’auteur. Préparation du dépôt sans flashage de carte ni publication distante. Version livrée : `1.0.2`. Le format de stockage reste `CONFIG_VERSION = 1` ; la clé existante `appass` sert au mot de passe AP modifiable.
...
- Comparaison avec l’original : marqueur de licence, cible documentée et texte d’accès corrigés ; ajout ciblé du changement de mot de passe AP, de son enregistrement et de sa priorité au démarrage. Algorithmes de détection et alertes inchangés ; ajout du témoin LED dans les chemins de démarrage accepté et de supervision du scan actif, retrait du déclenchement par appareil surveillé.
...
- LED : contrôle de GPIO8 actif LOW, extinction après impulsion sans attente bloquante, et déclenchement indépendant des appareils surveillés. Source documentaire GPIO/polarité liée dans les deux README ; aucun essai électrique sur carte.
...
- Programme : **1 493 499 octets**, soit 47 % des 3 145 728 octets de la partition.
- Variables globales : **64 812 octets**, soit 19 % des 327 680 octets de mémoire dynamique annoncés par l’outil.
- Mémoire restante annoncée : **262 868 octets** avant allocations d’exécution ; ce chiffre n’est pas une mesure de heap en fonctionnement.

Archive vérifiée après création : neuf fichiers autorisés uniquement, dont la bannière PNG, `.gitignore` inclus, intégrité ZIP et correspondance octet par octet avec les fichiers du dossier de livraison. Aucun binaire firmware, log, export NVS ou source originale inclus. Empreinte SHA-256 fournie à côté du ZIP.
